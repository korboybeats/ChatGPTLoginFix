...
- (void)performActivity {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Paste ChatGPT Session Token" message:@"Paste only the session token value:" preferredStyle:UIAlertControllerStyleAlert];
    
    [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
        textField.placeholder = @"Paste your cookie here";
    }];
    
    UIAlertAction *injectAction = [UIAlertAction actionWithTitle:@"Inject" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        NSString *token = alert.textFields.firstObject.text;
        if (token.length > 0) {
            NSString *cookieString = [NSString stringWithFormat:@"__Secure-next-auth.session-token=%@; Domain=.chatgpt.com; Path=/; Secure; HttpOnly; SameSite=Lax; Max-Age=31536000", token];
